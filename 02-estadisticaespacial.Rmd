...
Dado un dominio $D\subset \mathbb{R}^n$ y un espacio de probabilidad $(\Omega,\mathcal{A},P)$, una función aleatoria es una función de dos variables $Z(x,\omega)$ tal que para cada $x\in D$, $Z(x,.)$ es una variable aleatoria en $(\Omega,\mathcal{A},P)$. Por otro lado, $Z(.,\omega)$, definido en $D$ como la sección de la función aleatoria en $\omega \in \Omega$ es una realización de la función aleatoria. La función aleatoria $Z(x,\omega)$ se denota como $Z(x)$ y una realización es representada por $z(x)$ \cite{GeoJean}.

### Proceso Estocástico

Según \cite{GeoJean}, una función aleatoria es llamada proceso estocástico cuando $x$ varia en un espacio unidimensional como el tiempo; por otro lado, si $x$ varia en más de una dimensión es conocido como campo aleatorio.
...
donde $D \subset \mathbb{R}^d$ es el conjunto formado por todas las ubicaciones espaciales \cite{marta}.
...
donde $D \subset \mathbb{R}$ es el conjunto formado por todos los instantes de tiempo \cite{marta}.
...
donde el conjunto $D \subset \mathbb{R}^d$ es el conjunto de todas las ubicaciones espaciales y el conjunto $D' \subset \mathbb{R}$ es el conjunto de todos los instantes de tiempo. Los conjuntos $D$ y $D'$ pueden ser continuos o discretos, fijos o aleatorios \cite{marta}.
...
Es el conjunto de métodos aplicados a datos espaciales, donde las ubicaciones espaciales $s$ provienen de un conjunto continuo y fijo $D \subset \mathbb{R}^d$; cabe resaltar que el propósito de la geoestadística es la interpolación. Además, si el conjunto $D$ no es continuo se puede llegar a obtener predicciones carentes de sentido, y se considera $D$ fijo pues los puntos en el espacio se seleccionan a conveniencia o bajo un esquema de muestreo probabilístico \cite{Ramon}.

### Datos de Área

En este tipo de datos las ubicaciones espaciales pertenecen a un conjunto discreto contable y fijo $D \subset \mathbb{R}^d$. Las ubicaciones pueden estar espaciadas regular o irregularmente, usualmente son irregulares pues sus separaciones no siguen un patrón predecible.\cite{Cressi}.

### Patrones Puntuales

En este caso, las ubicaciones espaciales pertenecen a un conjunto discreto o continuo y aleatorio $D\subset \mathbb{R}^d$. Con este tipo de dato se realizan análisis con el propósito de determinar si la distribución de las mediciones en la región es aleatoria, agregada o uniforme \cite{Ramon}.

### Datos Georreferenciados

En este caso, las variables de interés tienen asociadas las coordenadas de las ubicaciones donde fueron medidas, las que pueden ser geográficas, planas o cartesianas \cite{Ramon}.

## Geoestadística

La palabra geoestadística fue establecida por Hart en el año de 1954. El significado de esta palabra se lo puede deducir a partir de su prefijo geo, que hace referencia a la tierra; por lo tanto, la geoestadística es una rama de la estadística enfocada en el análisis y la modelización de la variabilidad espacial de fenómenos que ocurren dentro de una zona geográfica; además, es considerada como una disciplina híbrida entre Geología, Matemática, Estadística y Minería de datos \cite{Cressi}.
...
En el caso de que las mediciones sean realizadas en una superficie; es decir, $d=2$, $Z(s)$ puede asociarse a la variable aleatoria ligada a ese punto del plano, $s$ representa las coordenadas geográficas y $Z$ la variable en cada una de ellas. La medición de esta variable aleatoria puede representar la magnitud de una variable ambiental medida en un conjunto de coordenadas en la región de estudio \cite{Ramon}.

Un proceso estocástico espacial se caracteriza por su distribución de probabilidad de dimensión finita; es decir, la distribución de probabilidad conjunta de un conjunto de variables $Z(s_1),...,Z(s_k)$ para todo $k \in \mathbb{N}$ y para todos los puntos $s_1,...s_k \in D$ \cite{marta}.
...
\end{align*}
```
En cada sitio $s$ dado, $\mu(s)$ representa la media alrededor de la cual se distribuyen los valores tomados por las realizaciones de la funciones aleatoria \cite{Ramon}. Si la esperanza del campo aleatorio varía con la ubicación espacial, se le conoce como deriva o \textit{drift} del campo aleatorio.
...
```{=tex}
\begin{align*}
  F_{Z_1,...,Z_n}(s_1,...,s_n)=F_{Z_1,...,Z_n}((s_1+h),...,(s_n+h))  
\end{align*}
```
Nótese que esta condición es muy restrictiva, por lo que normalmente se relaja a condiciones de segundo orden, las que limitan el supuesto de estacionariedad a los dos primeros momentos del proceso estocástico o campo aleatorio \cite{montero}.
...
-   La covarianza existe para todo par de variables aleatorias, $Z(s)$ y $Z(s+h)$, y solo depende del vector de separación $h$ entre las ubicaciones $s$ y $s+h$; es decir, depende de la dirección y la distancia de separación, y no de sus ubicaciones absolutas y se tiene que:
    ...
    ```{=tex}
    \begin{align*}
        Cov(Z(s),Z(s+h))=C(h),\forall s\in D \ y\ h  
      \end{align*}
    ```
...
```{=tex}
\begin{align*}
    \gamma(h)&=\frac{1}{2}V(Z(s+h)-Z(s)), \quad \text{con} \ \gamma(h)=\gamma(s+h,s)\\
&=\frac{1}{2}(V(Z(s+h))+V(Z(s))+2C(Z(s+h),Z(s)))\\
...
Cuando el proceso estocástico o campo aleatorio $\{Z(s): s\in D\}$ tiene media no constante y varia con la ubicación espacial, y sus incrementos de primer orden $Z(s+h)-Z(s)$ son no estacionarios, se dice que la función aleatoria es no intrínseca \cite{montero}.
...
\begin{align*}
    Cov(Z(s),Z(s+h)&= C(||h||))\\
    \frac{1}{2} Var(Z(s+h)-Z(s))&=\gamma(||h||)
\end{align*}
```
el proceso posee función de covarianza y/o semivarianza isotrópica. La estacionariedad posibilita combinar pares de datos con la misma diferencia en magnitud de separación de las coordenadas, y si los vectores de diferencias tienen la posibilidad de ser reemplazados con distancias escalares, entonces el campo se dice isotrópico \cite{marta}. Si la correlación entre los datos no es dependiente de la dirección en la que esta se calcule se plantea que el fenómeno es isotrópico \cite{Ramon}; en caso opuesto un campo aleatorio que es estacionario pero no isotrópico se desenvuelve de forma distinta según las diferentes direcciones del espacio; por lo cual, no es suficiente conocer cuánto permanecen distanciadas las ubicaciones, sino además es necesario conocer la orientación de esa distancia; a dichos campos se los conoce como campos aleatorios anisotrópicos \cite{marta}.

### Anisotropía

### Correlación Espacial

El análisis estructural es la primera etapa en el desarrollo de un análisis geoestadístico; a partir de este se puede expresar la estructura de dependencia espacial o correlación que existe entre los datos medidos de una variable mediante funciones como el variograma y/o covariograma \cite{Ramon}.
...
-   Es una función definida positiva, en términos de diferencia entre coordenadas $s_i-s_j=h$:
    ...
    Esta condición es la más importante y necesaria para que una función de covarianza esté bien definida \cite{montero}.
...
\end{align*}
```
En la práctica se utiliza el semivariograma en lugar del covariograma puesto que este no necesita conocer la media de la función aleatoria \cite{montero}.
...
\end{align*}
```
donde $\gamma'(h)$ es el modelo de variograma, $c_p$ es la silla parcial y $c_0$ es el efecto pepita \cite{marta}.

```{=tex}
\begin{figure}[H]
\centering
\includegraphics[width=13cm,height=7cm]{figuras/otros/variograma_partes.PNG}
\caption{Partes de un variograma}
\label{fig:pvar}
\end{figure}
```
...
\begin{figure}[H]
\centering
\includegraphics[width=6cm,height=3cm]{figuras/otros/distancia_h.png}
\caption{Distancia}
\label{fig:distanciah}
\end{figure}
```
Si $\hat{\gamma}$ es dependiente de la separación y orientación del par de ubicaciones descritos por el vector $h$, se tiene que \cite{hans}:
...
\end{align*}
```
La disimilitud es simétrica con respecto de $h$, pues es una cantidad al cuadrado, por lo que, su representación será mostrada utilizando todos los pares de datos de la muestra en el conjunto $D$. El gráfico de la disimilitud $\hat{\gamma}$ contra la separación espacial absoluta $h$, es llamado nube del variograma \cite{hans}.

La disimilitud con frecuencia se incrementa con la distancia, pues los puntos cercanos tienden a ser similares \cite{hans} (ver Figura \ref{varcloud}).

```{=tex}
\begin{figure}[H]
\centering
\includegraphics[width=10cm,height=5cm]{figuras/otros/var_cloud.png}
\caption{Disimilitud respecto de $h$.}
\label{varcloud}
\end{figure}
```
### Variograma Empírico

Teniendo en cuenta que el variograma $\gamma(h)$ es la varianza de la variable de incrementos $Z(s+h)-Z(s)$, el estimador clásico se basa en la estimación de esta varianza mediante el método de momentos y está definido para un vector de separación $h$ de la siguiente manera:

```{=tex}
\begin{align*}
  \hat{\gamma}(h)=\frac{1}{2|N(h)|}\sum_{N(h)}(Z(s+h)-Z(s))^2, \ h\in \mathbb{R}^d  
\end{align*}
```
donde:
...
Al ser un estimador de la varianza muestral es sensible a datos atípicos; es por ello que \cite{Cressi} propuso estimadores resistentes a datos atípicos, asumiendo normalidad marginal del campo aleatorio \cite{marta}; sin embargo, la diferencia entre estos estimadores es pequeña \cite{Cressi}.
...
Siendo $T(h)$ una región de tolerancia alrededor de $h$; es decir, $T(h)=[h-\Delta h,h+\Delta h]$ en el caso unidiminesional. En el caso de dimensión dos o tres, existen tolerancias sobre la longitud de $h$ y sobre su orientación \cite{emery} (véase Figura \ref{fig:reg_tol}):

```{=tex}
\begin{figure}[H]
\centering
\includegraphics[width=10cm,height=5cm]{figuras/otros/tolerancia.png}
\caption{Región de tolerancia $T(h)$ alrededor del vector $h$.}
\label{fig:reg_tol}
\end{figure}
```
El ancho de banda limita la división del cono de tolerancia a una expansión máxima. En el espacio de tres dimensiones, se introducen dos anchos de banda, horizontal y vertical \cite{emery}.

#### Propiedades del variograma empírico {.unnumbered}

-   El variograma empírico $\hat{\gamma}(h)$ es un estimador insesgado del variograma teórico \cite{montero}:
...
-   La varianza relativa es un indicador de robustez:
    ...
    Los principales factores que influyen en la varianza relativa son \cite{emery}:
...
\begin{figure}[H]
\centering
\includegraphics[width=15cm,height=8cm]{figuras/otros/var_comp.png}
\caption{Comportamientos del variograma en el origen.}
\label{var_comp}
\end{figure}
```
...
Como se vio anteriormente, las variables aleatorias $Z(s)$ y $Z(s+h)$ están correlacionadas, si la longitud del vector de separación $h$ es inferior a la distancia $\phi$ o alcance; más allá de $|h|=\phi$, el variograma es constante e igual a su silla, y las variables $Z(s)$ y $Z(s+h)$ son independientes. Los variogramas con silla, se denominan \textit{modelos de transición} (ver Figura \ref{var_sill}) \cite{emery}.

```{=tex}
\begin{figure}[H]
\centering
\includegraphics[width=10cm,height=5cm]{figuras/otros/vari_sill.png}
\caption{Lado izquierdo: Variograma con silla y alcance. Lado derecho: Variograma sin silla y  alcance.}
\label{var_sill}
\end{figure}
```
Existen casos en los cuales el variograma crece infinitamente una vez la distancia se incrementa y no posee ni silla ni rango; en este caso, la varianza es infinita $C(0)=\infty$ y no existe la función de covarianza ni el correlograma. Cuando se tiene ausencia de silla, es posible que sea una consecuencia del efecto de micro-escala \cite{emery}.

### Criterios para la estimación y ajuste del variograma

#### Tendencia: {.unnumbered}

Es fundamental verificar que los datos no presenten tendencia, puesto que cuando la media del proceso $\mu(s)$ no es constante; es decir, el proceso no es estacionario, el variograma empírico calculado a partir de las observaciones es erróneo; este comportamiento de tendencia se puede comprobar mediante un gráfico de dispersión de la variable de respuesta frente a cada coordenada espacial (ver Figura \ref{coord_trend}). En caso de que se evidencie tendencia se sugiere que se incluya un modelo de superficie de tendencia para la media que varía espacialmente, cuando una superficie de tendencia es incluida en el modelo, las dos coordenadas espaciales deben contribuir en este, puesto que la orientación de la región de estudio es arbitraria. En la práctica; al trabajar con datos geográficos de gran escala, se puede esperar que ciertas variables relacionadas a un ambiente físico muestren dependencia en la latitud \cite{peter}.

```{=tex}
\begin{figure}[H]
\centering
\includegraphics[width=10cm,height=5cm]{figuras/otros/coord_trend.png}
\caption{Tendencia de los valores con respecto a las coordenadas.}
\label{coord_trend}
\end{figure}
```
Cuando la función de media no es constante, el variograma empírico atribuye de manera errónea la variación inducida por esta a la estructura de convarianza de gran escala en el proceso no observado $Z(s)$. Una solución es estimar $\mu(s)$ mediante un modelo de superficie de tendencia o, si la información de la covariable está disponible, mediante un modelo general de regresión, y utilizar los residuos $R_i=Z_i-\hat{\mu}(s_i)$ en lugar de las observaciones para el cálculo del variograma empírico \cite{peter}. Este comportamiento se puede visualizar en la Figura \ref{rcoord_trend}.

```{=tex}
\begin{figure}[H]
\centering
\includegraphics[width=10cm,height=5cm]{figuras/otros/rcoord_trend.png}
\caption{Tendencia de los residuos con respecto a las coordenadas.}
\label{rcoord_trend}
\end{figure}
```
#### Establecer la variable correcta: {.unnumbered}

En la estimación del variograma influye la distribución de los datos, pues estos pueden estar sesgados o tener valores extremos altos o bajos y por tanto su variograma calculado suele presentar un comportamiento errático. Para abordar este problema es recomendable transformar los datos a un espacio Normal o Gaussiano \cite{garten}. Ver Figura \ref{den_var}.

```{=tex}
\begin{figure}[H]
\centering
\includegraphics[width=10cm,height=5cm]{figuras/otros/den_var.png}
\caption{Arriba: Variograma empírico calculado a partir de los datos originales. Abajo: Variograma empírico calculado a partir de los residuos.}
\label{den_var}
\end{figure}
```
#### Verificación de isotropía: {.unnumbered}

Como bien se conoce, se debe verificar la estacionariedad de los datos, puesto que este influye de manera directa en la estimación del variograma empírico; teniendo en cuenta que un proceso espacial puede estar definido en varias direcciones, este criterio se debe verificar para todas estas. No obstante, en la práctica la isotropía se estudia por medio del cálculo de funciones de autocovariaza o de semivarianza muestrales en algunas direcciones que habitualmente son: $0^o,45^o, 90^o$ y $135^o$, ver Figura. \ref{iso_dir}.

```{=tex}
\begin{figure}[H]
\centering
\includegraphics[width=10cm,height=5cm]{figuras/otros/iso_dir.png}
\caption{Comportamiento del variograma direccional.}
\label{iso_dir}
\end{figure}
```
...
\end{align*}
```
Una de las razones principales para usar las funciones de verosimilitudes marginales es que aunque al inicio no se cumpla el supuesto requerido, es posible aproximarse a este por medio de alguna transformación \cite{marta}.
...
\end{align*}
```
con $b_l=(a_l)^2\geq 0$, donde $b_l$ es la contribución en varianza del correspondiente variograma $\gamma'_l(h)$ \cite{marta}.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=10cm,height=5cm]{figuras/otros/sph_var.png}
    \caption{Izquierda: Variable regionalizada. Derecha: Variograma esférico.}
    \label{sph_var}
\end{figure}
```
...
\begin{figure}[h]
    \centering
    \includegraphics[width=10cm,height=5cm]{figuras/otros/pepita_var.png}
    \caption{Izquierda: Variable regionalizada. Derecha: Variograma pepita puro.}
    \label{sph_var}
\end{figure}
```
**Modelo exponencial**
...
\begin{figure}[H]
    \centering
    \includegraphics[width=10cm,height=5cm]{figuras/otros/exp_var.png}
    \caption{Izquierda: Variable regionalizada. Derecha: Variograma exponencial.}
    \label{exp_var}
\end{figure}
```
**Modelo Gaussiano**
...
\begin{figure}[H]
    \centering
    \includegraphics[width=10cm,height=5cm]{figuras/otros/gau_var.png}
    \caption{Izquierda: Variable regionalizada. Derecha: Variograma Gaussiano.}
    \label{gau_var}
\end{figure}
```
**Modelo Cúbico**
...
\begin{figure}[H]
    \centering
    \includegraphics[width=10cm,height=5cm]{figuras/otros/cub_var.png}
    \caption{Izquierda: Variable regionalizada. Derecha: Variograma cúbico.}
    \label{cub_var}
\end{figure}
```
**Modelo Estable**
...
\begin{figure}[H]
    \centering
    \includegraphics[width=10cm,height=5cm]{figuras/otros/estable_var.png}
    \caption{Izquierda: Variable regionalizada. Derecha: Variograma estable.}
    \label{estable_var}
\end{figure}
```
...
\begin{figure}[H]
    \centering
    \includegraphics[width=10cm,height=5cm]{figuras/otros/caugen_var.png}
    \caption{Izquierda: Variable regionalizada. Derecha: Variograma Cauchy generalizado.}
    \label{caugen_var}
\end{figure}
```
...
\begin{figure}[H]
    \centering
    \includegraphics[width=10cm,height=5cm]{figuras/otros/mat_var.png}
    \caption{Izquierda: Variable regionalizada. Derecha: Variograma Matérn.}
    \label{mat_var}
\end{figure}
```
...
\begin{figure}[H]
    \centering
    \includegraphics[width=10cm,height=5cm]{figuras/otros/bes_var.png}
    \caption{Izquierda: Variable regionalizada. Derecha: Variograma J-Bessel.}
    \label{bes_var}
\end{figure}
```
...
\begin{figure}[H]
    \centering
    \includegraphics[width=10cm,height=5cm]{figuras/otros/sin_var.png}
    \caption{Izquierda: Variable regionalizada. Derecha: Variograma seno cardinal.}
    \label{sin_var}
\end{figure}
```
...
$$  
  Este variograma no posee ni silla ni rango, sino que crece indefinidamente. Si $\alpha$ es cercano a cero, se dice que es un variograma de efecto pepita, si $\alpha$ es cercano a dos tiene un comportamiento parabólico y si $\alpha$ es igual a uno presenta un comportamiento lineal. Para $\alpha\geq 2$ la condición $\lim_{h\to \infty}\frac{\gamma||h||}{h^2}=0$ no se satisface; es decir, el modelo no es intrínsecamente estacionario.
    ...
    \begin{figure}[H]
          \centering
          \includegraphics[width=10cm,height=5cm]{figuras/otros/pow_var.png}
          \caption{Izquierda: Variable regionalizada. Derecha: Variograma potencia.}
          \label{pow_var}
    \end{figure}
    ```
...
donde $Z(s)$ es un vector fila de $p$ procesos estocásticos espaciales o campos aleatorios estacionarios de segundo orden, $\bm{\mu}=E[Z(s)]$ y $M$ es una matriz simétrica definida positiva de tamaño $p\times p$ utilizada como métrica en el cálculo de similaridades \cite{borou}.
...
El variograma multivariado representa la esperanza matemática de una medida de disimilitud cuadrática, y la función de autocovarianza multivariada representa la esperanza matemática de una medida de similaridad multivariante \cite{borou}.
...
\end{align*}
```
donde $d_{ij}$ es la disimilitud entre las muestras $i$ y $j$ calculadas con una métrica dada y $N(h)$ como se definió anteriormente \cite{borou}.
...
\end{align*}
```
el criterio de bondad de ajuste es la suma de cuadrados ponderados (SCP) de todos los términos de la matriz de errores $\hat{\Gamma}(h)-\Gamma(h)$ y se suman sobre el conjunto de retardos $J$ utilizados para el ajuste; es decir:%, específicamente es la norma Euclideana:
...
```{=tex}
\begin{align*}
    B_k=U_k\nabla_k U_k^{'}\quad \text{con} \quad U_k^{'}VU_k=I_p
\end{align*}
```
...
\end{align*}
```
donde $\nabla_k^{+}$ es la matriz $\nabla_k$ en el cual los valores propios negativos son reemplazados por ceros. Este algoritmo iterativo siempre converge y la solución no depende del punto de partida \cite{peter}.
